...
//
// Windows' SetWindowDisplayAffinity with WDA_EXCLUDEFROMCAPTURE, reached through
// Wails' ContentProtectionEnabled, applied for as long as the user asks for it.
// Captures see a blank rectangle where the window is; the window itself looks
// normal on the physical screen.
//
// Windows opt in with Follow. Anything that protects itself unconditionally must
// stay out: this package would switch that protection off the moment the
...
// SetContentProtection here would deadlock. The call site creates its window on
// the main thread, before Run() is pumping at all, while SetContentProtection
// dispatches to the main thread and blocks until it answers.
